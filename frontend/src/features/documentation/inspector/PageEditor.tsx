      ...
      <Plate editor={editor} onChange={write}>
        <MentionSourceProvider candidates={candidates} onSearchMentions={onSearchMentions}>
          <FixedToolbar className="justify-start gap-0.5 border-b border-border px-2 py-1.5">
            {/*
              Radix's tooltip context, which Plate's controls expect. This project's own
              tooltip is built on Base UI, so the editor's chrome keeps its own provider
          ...
          <div className="relative min-h-0 flex-1 overflow-y-auto">
            {mode === 'rich' ? (
              <PlateContent
                ...
                className="documentation-markdown min-h-full px-6 py-4 text-sm leading-relaxed focus-visible:outline-none"
